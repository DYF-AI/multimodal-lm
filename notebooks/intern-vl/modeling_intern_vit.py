...
class InternVisionEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        """
        hidden_states = hidden_states + self.drop_path1(self.attn(self.norm1(hidden_states)) * self.ls1)  # 进行layer_norm, attention计算, dropout
...
class InternVisionEncoder(nn.Module):
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):  # 遍历每一层的InternVisonEncoderLayer
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = encoder_layer(  # 每一层的输出, 循环
                    hidden_states,
                )
    ...
    def forward(
        ...
        if pixel_embeds is not None:
            ...
            if len(pixel_values.shape) == 4:
                hidden_states = self.embeddings(pixel_values)  # (n, seq_len, dim)
            else:
                raise ValueError(f'wrong pixel_values size: {pixel_values.shape}')
        encoder_outputs = self.encoder(    # 视觉编码器
